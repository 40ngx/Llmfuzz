...
use async_openai::{
    config::{AzureConfig, GroqConfig, OpenAIConfig},
    types::{
...
// fn get_groq_client() -> Result<&'static Client<GroqConfig>> {
//     // read OpenAI API key form the env var (OPENAI_API_KEY).
//     pub static CLIENT: OnceCell<Client<GroqConfig>> = OnceCell::new();
//     let client = CLIENT.get_or_init(|| {
//         let http_client = reqwest::ClientBuilder::new().connect_timeout(Duration::from_secs(300)).timeout(Duration::from_secs(60)).build().unwrap();
//         // let client = if let Some(proxy) = get_openai_proxy() {
//         //     Client::new().with_http_client(proxy)
//         // } else {
//         //     Client::new()
//         // };
//          let config = GroqConfig::new()
//             .with_api_base("https://api.groq.com/openai/v1")
//             .with_api_key("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");
...
fn get_client() -> Result<&'static Client<OpenAIConfig>> {
    ...
    let client = CLIENT.get_or_init(|| {
        let http_client = reqwest::ClientBuilder::new().connect_timeout(Duration::from_secs(300)).timeout(Duration::from_secs(300)).build().unwrap();
        // let client = if let Some(proxy) = get_openai_proxy() {
        //     Client::new().with_http_client(proxy)
        // } else {
        //     Client::new()
        // };
         let config = OpenAIConfig::new()
            // .with_api_base("http://222.20.126.175:11434/v1")
            // .with_api_base("ollama");
            .with_api_base("http://186.32.16.1:11434/v1")
            .with_api_key("XXXXXX");
            // .with_api_base("http://20.51.110.55:5005/jyhlab/v1")
            // .with_api_key("");
...
pub fn get_azure_client() -> Result<&'static Client<AzureConfig>> {
    // read OpenAI API key form the env var (OPENAI_API_KEY).
    pub static CLIENT: OnceCell<Client<AzureConfig>> = OnceCell::new();
    let client = CLIENT.get_or_init(|| {
        ...
        let config = AzureConfig::new()
            .with_api_base("https://gpt-jiaxuan.openai.azure.com")
            .with_api_key("XXXXXX")
            .with_deployment_id("gpt-35-turbo-16k")
